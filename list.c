...
void *list_remove(List *l, int idx) {
  if (idx >= l->len) {
    fprintf(stderr,
            "Error: cannot remove item at index %d from list of length %d.\n",
            idx, l->len);
    exit(EXIT_FAILURE);
  }
...
void list_insert(List *l, void *item, int idx) {
  if (idx > l->len) {
    fprintf(stderr,
            "Error: cannot insert item at index %d into list of length %d.\n",
            idx, l->len);
    exit(EXIT_FAILURE);
  }
...
bool list_contains(List *l, void *item,
                   bool (*equality_fn)(const void *, const void *)) {
  for (int i = 0; i < l->len; i++) {
    if (equality_fn(l->data[i], item)) {
      return true;
    }
  }
